...
export default function Forums() {
  ...
  const [newThreadOnlyResearchersCanReply, setNewThreadOnlyResearchersCanReply] = useState(false);
  ...
  const [showShareToDiscoveryModal, setShowShareToDiscoveryModal] = useState(false);
  ...
  function mergeDummyThreads(apiThreads, promotedDummyKeys = []) {
    ...
    const dummies = buildDummyThreadsForMerge(communities, selectedCommunity?.slug ?? null);
  ...
  const threadIdsString = useMemo(() => {
    return threads.map(t => t._id).filter(Boolean).join(',');
  }, [threads]);
  ...
  useEffect(() => {
    ...
    const userThreads = threads.filter(
      (t) => {
        const authorId = t.authorUserId?._id?.toString() || t.authorUserId?.toString();
  ...
  function openUserProfileModal(userId) {
    ...
    if (isDummyUserId(uid)) {
      setUserProfileModalData({
        ...
        user: { displayName: "Sample participant", username: (uid || "").toString() },
      });
      setUserProfileModalLoading(false);
      return;
    }
  ...
  async function loadThreads() {
    ...
    try {
      ...
      params.set("excludeResearcherForum", selectedCommunity.communityType === "researcher" ? "false" : "true");
  ...
  async function loadThreadsBySubcategory() {
    ...
    try {
      ...
      const isResearcherCommunity = selectedCommunity?.communityType === "researcher";
      params.set("excludeResearcherForum", isResearcherCommunity ? "false" : "true");
      // Tag filtering is done client-side now
  ...
  async function loadResearcherRepliedThreads() {
    ...
    try {
      ...
      let filteredThreads = allThreads.filter((thread) => 
        thread.hasResearcherReply === true
      );
      ...
      if (sortBy === "recent") {
        ...
        filteredThreads.sort((a, b) => {
          const aScore = (a.upvotes?.length || 0) - (a.downvotes?.length || 0) + (a.replyCount || 0) * 2;
          const bScore = (b.upvotes?.length || 0) - (b.downvotes?.length || 0) + (b.replyCount || 0) * 2;
  ...
  async function postThread() {
    ...
    try {
      ...
      setShareMessage(`I just posted a question in ${communityToUse.name}: "${trimmedTitle}"`);
  ...
  function openShareModal(thread) {
    const communityToUse = thread.communityId || thread.categoryId || selectedCommunity;
    setNewlyCreatedThread({
      _id: thread._id,
      title: thread.title,
      body: thread.body,
    ...
    setShareMessage(`I just posted a question in ${communityToUse?.name || "the forums"}: "${thread.title}"`);
    setShowShareToDiscoveryModal(true);
  }
  ...
  async function shareToDiscovery() {
    ...
    try {
      ...
      const response = await fetch(`${base}/api/posts`, {
        ...
        body: JSON.stringify({
          content: shareMessage.trim() || `I just posted a question in ${threadToShare.community?.name || "the forums"}: "${threadToShare.title}"`,
  ...
  async function deleteReply(threadId, replyId) {
    ...
    try {
      ...
      setExpandedThreads((prev) => {
        ...
        if (updated[threadId]?.replies?.length) {
          updated[threadId] = {
            ...
            replyCount: Math.max(0, (updated[threadId].replyCount ?? 0) - deletedCount),
          };
        }
        return updated;
      });
      setThreads((prev) =>
        prev.map((t) =>
          t._id === threadId
            ? { ...t, replyCount: Math.max(0, (t.replyCount ?? 0) - deletedCount) }
            : t
        )
      );
      setEditingReplyId(null);
  ...
  const threadsWithVoteStates = useMemo(() => {
    ...
    const userId = (user?._id || user?.id)?.toString();
    if (!userId) return threads.map(t => ({ ...t, isUpvoted: false, isDownvoted: false }));
    
    return threads.map(thread => ({
      ...thread,
      isUpvoted: (thread.upvotes || []).some(id => String(id) === userId) || false,
      isDownvoted: (thread.downvotes || []).some(id => String(id) === userId) || false,
    }));
          ...
          <div className="flex items-start justify-between mb-2 gap-2">
            <div className="flex items-center gap-2 flex-1 min-w-0">
              <button
                ...
                style={{ backgroundColor: reply.authorUserId?.picture ? 'transparent' : '#2F3C96' }}
              ...
              >
                {reply.authorUserId?.picture ? (
                  <img
                    ...
                    alt={reply.authorUserId?.nameHidden && reply.authorUserId?.handle ? `@${reply.authorUserId.handle}` : getDisplayName(reply.authorUserId, "Anonymous")}
                ...
                <span className={`avatar-fallback ${reply.authorUserId?.picture ? 'hidden' : ''}`}>
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  >
                    {!reply.authorUserId?.nameHidden && (
                      <span className="text-sm font-medium">{getDisplayName(reply.authorUserId, "Anonymous")}</span>
                    )}
                    {reply.authorUserId?.handle && (
                      <span className={`text-xs ${reply.authorUserId?.nameHidden ? 'font-medium' : 'text-[#787878]'}`}>
                        @{reply.authorUserId.handle}
                      </span>
                    )}
                  </button>
                    ...
                    <div className="w-28 sm:w-36 shrink-0">
                      <CustomSelect
                        ...
                        options={[
                          ...
                          { value: "researcherReplied", label: "Researcher Replied" },
                    ...
                    {user && (
                      <button
                        onClick={() => {
                          ...
                          if (selectedConditionTag !== "All" && FORUM_SUBCATEGORY_TAGS.includes(selectedConditionTag)) {
                            ...
                            >
                              <div className="flex items-center gap-3 min-w-0">
                                ...
                                <h3 className="text-lg md:text-xl font-bold text-[#484848]">
                                  {category.name}
                                  <span className="text-[#787878] font-normal ml-1.5">({count})</span>
                                </h3>
                              </div>
                              <span className={`text-xs px-2 py-1 rounded-full shrink-0 ${colorClasses.badge}`}>
                              ...
                              <div className="p-6 border-t border-[#E8E8E8]">
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                  {filteredCommunities.map((community) => {
                                    const itemColor = community.color || headingColor;
                                    const hasSvg = community.iconSvg && community.iconSvg.trim();
                                    ...
                                    return (
                                      <div
                                        ...
                                        onClick={() => {
                                          if (followingIds.has(community._id) || community.isFollowing) {
                                            setSelectedCommunity(community);
                                            setViewMode("posts");
                                          } else {
                                      ...
                                      >
                                          ...
                                          {hasSvg ? (
                                            <span
                                              ...
                                              dangerouslySetInnerHTML={{
                                                __html: sanitizeSvgForIcon(community.iconSvg),
                                              }}
                                            />
                                          ) : (
                                            <IconStethoscope
                                        ...
                                        <div className="flex-1 min-w-0">
                                          ...
                                          <div className="flex items-center justify-between gap-2 mt-1">
                                            ...
                                            {(followingIds.has(community._id) || community.isFollowing) ? (
                                              <span
                    ...
                    {forumCategories.filter((cat) => {
                      ...
                      <div className="bg-white rounded-lg border border-[#E8E8E8] p-12 text-center">
                        ...
                        <h3 className="text-lg font-semibold text-[#2F3C96] mb-2">
                          {forumCategories.length === 0 ? "No categories yet" : "No matches"}
                        </h3>
                  ...
                  <div className="bg-white rounded-xl border border-[#E8E8E8] p-4 mb-6 shadow-sm overflow-hidden">
                    ...
                    <div className="border-t border-[#E8E8E8] pt-4">
                      ...
                      <p className="text-xs text-[#787878] mt-2 hidden sm:block">
                        Filter discussions by tags/keywords and conditions & topics for better findability.
                      </p>
                    </div>
                  </div>
                )}
                                      ...
                                      {(thread.replyCount || 0) === 0 ? (
                                        <>
                                         <Clock className="w-4 h-4 shrink-0" aria-hidden />
                                          <span>{thread.onlyResearchersCanReply ? "Awaiting researcher reply" : "Awaiting reply"}</span>
                                        </>
                                      ) : (
                                        <>
                                    ...
                                    {user && (
                                      <>
                                        ...
                                        {(() => {
                                          ...
                                          const isShared = sharedThreadIds.has(thread._id);
                                          
                                          // Don't show button if not author or already shared
                                          if (!isAuthor || isShared) return null;
                                        ...
                                        {(() => {
                                          ...
                                          const currentUserId = (user._id || user.id)?.toString();
                                          const isAuthor = authorId === currentUserId;
                                          if (!isAuthor || thread.isDummy) return null;
                    ...
                    <section className="space-y-3">
                      ...
                      <div className="flex items-start gap-3 pt-1">
                        <input
                          ...
                          onChange={(e) => setNewThreadOnlyResearchersCanReply(e.target.checked)}
                          className="mt-1 h-4 w-4 rounded border-[#E8E8E8] text-[#2F3C96] focus:ring-[#2F3C96]/20"
                        />
                        <label htmlFor="newThreadOnlyResearchersCanReply" className="text-sm text-[#484848] cursor-pointer">
          ...
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-hidden flex flex-col">
              ...
              <div className="flex items-center justify-between p-6 border-b border-[#E8E8E8]">
                <div>
                  <h2 className="text-xl font-bold text-[#2F3C96]">Share to Discovery</h2>
                  <p className="text-sm text-[#787878] mt-1">
                    Increase visibility by sharing your forum question to your discovery feed
                  </p>
                </div>
                <button
                  onClick={() => {
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-4">
                ...
                <div className="border border-[#E8E8E8] rounded-lg p-4 bg-[#F9F9F9] hover:border-[#2F3C96]/30 transition-colors cursor-pointer"
                  onClick={() => {
                    setShowShareToDiscoveryModal(false);
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {newlyCreatedThread.tags && newlyCreatedThread.tags.length > 0 && (
                        <div className="flex flex-wrap gap-1.5 mt-2">
                          {newlyCreatedThread.tags.slice(0, 3).map((tag, idx) => (
                            <span
                              key={idx}
